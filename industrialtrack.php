            ...
            foreach ($json['commiteeMembers']['industry_track'] as $key => $value) {
                ...
                <div class="col-md-4">
                    <?php if (!is_null($value['website'])) { ?>
                        <a href="<?php echo $value['website'] ?>" target="_blank"><?php echo $value['name'] ?></a><?php if ($value['name'] == 'Dongmei Zhang' || $value['name'] == 'Neha Rungta') { echo ' <span style="font-size: 18px;">(co-Chair)</span>';} ?>
                    <?php } else { ?>
