<details>
    <summary>Language Models</summary>
    <div class="chair">Session Chair: Yasu Kamei</div>
    <p>
        <ul>
            <li>
                <div class="program_paper">
                    <?= paper() ?> 
                    <span class="title">Syntax and Sensibility: Using language models to detect and correct syntax errors</span>
                    <span class="authors">Eddie Antonio Santos, Joshua Charles Campbell, Dhvani Patel, Abram Hindle, José Nelson Amaral</span>
                </div>
            </li>
            
            <li>
                <div class="program_paper">
                    <?= paper() ?> 
                    <span class="title">A Deep Neural Network Language Model with Contexts for Source Code</span>
                    <span class="authors">Anh Nguyen, Trong Nguyen, Hung Phan and Tien Nguyen</span>
                </div>
            </li>
        </ul>
    </p>
</details>
